...
public class PolygonConcavityIndex {
    public int solution(Point2D[] points) {
        ...
        for (int i = 0; i < n; i++) {
            // Calculate cross-product for triplet points[i], points[(i+1)%n], points[(i+2)%n]
            Point2D p1 = points[i];
            Point2D p2 = points[(i + 1) % n];
            Point2D p3 = points[(i + 2) % n];

            long crossProduct = cross(p1, p2, p3);
            if (crossProduct > 0) {
                hasPositive = true; // Counterclockwise turn
            } else if (crossProduct < 0) {
                hasNegative = true; // Clockwise turn
            }

            // If both positive and negative turns exist, the polygon is concave
            if (hasPositive && hasNegative) {
                return (i + 1) % n; // Return the index of a vertex inside the convex hull
            }
        }

        // If all turns are consistent, the polygon is convex
        return -1;
    }

    private long cross(Point2D p1, Point2D p2, Point2D p3) {
